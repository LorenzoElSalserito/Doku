  ...
  it('exports a typographic PDF through Pandoc and LuaLaTeX', async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        ...
        '--no-highlight',
        expect.stringMatching(/^--include-in-header=.*latexPreamble\.tex$/),
        expect.stringMatching(/^--lua-filter=.*tableWidths\.lua$/),
        expect.stringMatching(/^--variable=mainfontoptions:BoldFont=/),
        '--output',
        outputPath,
      ]),
      expect.objectContaining({
