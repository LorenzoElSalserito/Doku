...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (['--major', '--minor', '--patch'].includes(arg)) options.level = arg.slice(2)
...
function updatePackageJson(raw, version, desktop = false) {
  const pkg = JSON.parse(raw); pkg.version = version
  if (desktop) {
...
function run(argv) {
  ...
  const rootRaw = fs.readFileSync(p.packageJson, 'utf8'); const desktopRaw = fs.readFileSync(p.desktopPackageJson, 'utf8'); const lockRaw = fs.readFileSync(p.packageLock, 'utf8')
  const fromVersion = JSON.parse(rootRaw).version; meta.parseVersion(fromVersion)
  const pending = fs.existsSync(p.pendingRelease) ? meta.readJson(p.pendingRelease) : null
  const skip = Boolean(process.env.CI) || process.env.DOKU_NO_BUMP === '1'
  ...
  let changelog = meta.readChangelog(); let nextHistory = history; let entries = []; const now = new Date()
  if (bumping || !alreadyRecorded) {
    ...
    const author = JSON.parse(desktopRaw).author; const maintainer = `${author.name} <${author.email}>`
  ...
  const writes = [
    { file: p.packageJson, content: updatePackageJson(rootRaw, version) },
    { file: p.desktopPackageJson, content: updatePackageJson(desktopRaw, version, true) },
