...
const paths = {
  ...
  desktopPackageJson: path.join(repoRoot, 'apps', 'desktop', 'package.json'),
...
function parseEntries(body) {
  const entries = []; let category = null; let current = null
  const flush = () => { if (current?.trim()) entries.push(current.replace(/\s+/g, ' ').trim()); current = null }
...
function consolidateChangelog(markdown, version, date, entries) {
  ...
  const lines = markdown.split('\n')
  const released = [`## [${version}] - ${date}`, '', ...entries.map((entry) => `- ${entry}`), '']
...
function renderDebianChangelog(history, pkgName, maintainer) {
  return history.releases.map((r) => `${pkgName} (${r.version}) ${r.distribution || 'unstable'}; urgency=${r.urgency || 'medium'}\n\n${(r.entries?.length ? r.entries : [FALLBACK_ENTRY]).map((entry) => wrapEntry(entry)).join('\n')}\n\n -- ${r.maintainer || maintainer}  ${r.date}\n`).join('\n')
}

module.exports = { paths, UNRELEASED_HEADING, FALLBACK_ENTRY, readJson, parseVersion, formatVersion, bumpVersion, rfc2822, isoDate, readChangelog, extractUnreleasedBody, parseEntries, consolidateChangelog, readHistory, renderDebianChangelog, wrapEntry }
