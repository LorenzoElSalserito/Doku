...
function normalizeDescription(value) {
  ...
  if (synopsis.length > 80) {
    rest.unshift(synopsis); const cut = synopsis.slice(0, 80); synopsis = cut.slice(0, cut.lastIndexOf(' ')).replace(/[ ,;:.]+$/, '')
  }
...
function writeConffiles(root) {
  ...
  fs.writeFileSync(target, `${walk(etc, root).map((file) => `/${file.split(path.sep).join('/')}`).sort().join('\n')}\n`)
}
...
function verifyChangelog(text, version) {
  ...
  const file = path.join(os.tmpdir(), `doku-changelog-${process.pid}`); fs.writeFileSync(file, text)
...
function finalize(deb) {
  ...
  const pkg = meta.readJson(meta.paths.desktopPackageJson); const history = meta.readHistory()
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'doku-deb-'))
  try {
    ...
    const icon = path.join(meta.paths.repoRoot, 'apps', 'desktop', 'src', 'assets', 'icon.png'); installFile(root, `usr/share/pixmaps/${PKG_NAME}.png`, fs.readFileSync(icon))
